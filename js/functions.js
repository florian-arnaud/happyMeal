...
function getLocalStorageIngredients() {
  let ingredientsList = []
  const keys = Object.keys(localStorage);
  keys.forEach(key => {
    const value = localStorage.getItem(key);
    console.log(key);
    if (localStorage.getItem(key) == "Ingredients") {
      ingredientsList.push(key)

    }

  });
  const ingredientsBlob = new Blob([ingredientsList], { type: 'text/javascript' });

  return ingredientsBlob

}

function generateIngredientsFile()
{
  let file = getLocalStorageIngredients()
  const downloadLink = document.getElementById("ingredientListButton")
  downloadLink.href = URL.createObjectURL(file);
  downloadLink.download = 'courses.txt'; // Nom du fichier à télécharger
